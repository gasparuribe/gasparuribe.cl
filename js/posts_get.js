var sobre_esta_web_text=`
<div class="row">
    <div class="col-lg-12">
        <div class="mdl-card mdl-shadow--2dp mb-30">
        Para mí este sitio cumple una función similar a las redes sociales, pero un poco más propio y personalizado, ya que yo lo ensamble.
        Me es difícil sentirme cómodo por lo expuesto, pero al mismo tiempo me gusta que exista.
        No necesito esta web, no soy conocido y creo que puede ser negativo serlo, tampoco creo tener algún talento único o excepcional.
        </div>
    </div>
</div>
<div class="row">
    <div class="col-sm-6 mb-30">
      <div class="mdl-card mdl-shadow--2dp mb-30">
        <h2 class="mb-30">Servicios utilizados</h2>
        <ol style="padding-left: 20px;padding-right: 15px;">
          <li><h6>NIC Chile</h6> Gestor de dominios .cl</li>
          <li><h6>Cloudflare</h6> Host para configuraciones DNS</li>
          <li>
            <h6 style="display: inline;">GitHub Pages</h6> <a target="_blank" href="https://github.com/gasparuribe/gasparuribe.cl" style="color: #3c3cff;text-decoration: underline;font-size: 85%;" rel="nofollow">(Ver Codigo)</a><br>
            Host para frontend (img,html,js,css,...)
            <br>
            Ultimo cambio: <span id="gh_repo_date" style="font-size:90%;"></span>
          </li>
          <li><h6>AWS</h6> Host Lambda como backend</li>
          <li><h6>APIs</h6>
            <ul style="padding-left: 20px;padding-right: 15px;">
              <li><h6 style="display: inline;">GitHub</h6> <a target="_blank" href="https://docs.github.com/" style="color: #3c3cff;text-decoration: underline;font-size: 85%;" rel="nofollow">(Documentación)</a></li>
              <li><h6 style="display: inline;">Telegram</h6> <a target="_blank" href="https://core.telegram.org/" style="color: #3c3cff;text-decoration: underline;font-size: 85%;" rel="nofollow">(Documentación)</a></li>
              <li><h6 style="display: inline;">YouTube Data</h6> <a target="_blank" href="https://developers.google.com/youtube/v3/" style="color: #3c3cff;text-decoration: underline;font-size: 85%;" rel="nofollow">(Documentación)</a></li>
              <li><h6 style="display: inline;">Spotify</h6> <a target="_blank" href="https://developer.spotify.com/" style="color: #3c3cff;text-decoration: underline;font-size: 85%;" rel="nofollow">(Documentación)</a></li>
              <li><h6 style="display: inline;">Reddit</h6> <a target="_blank" href="https://www.reddit.com/dev/api/" style="color: #3c3cff;text-decoration: underline;font-size: 85%;" rel="nofollow">(Documentación)</a></li>
            </ul>
          </li>
          <li><h6>Hotjar</h6> Analytics</li>
        </ol>
      </div>
    </div>
  <div class="col-sm-6 mb-30">
    <div class="mdl-card mdl-shadow--2dp mb-30">
      <h2 class="mb-30">Recursos utilizados</h2>
      <ol style="padding-left: 20px;padding-right: 15px;">
        <li>
          <h6>Theme Matresume HTML</h6>
          Comprar o ver el demo en
          <a style="color:#3c3cff;text-decoration: underline;" target="_blank"
          href="https://themeforest.net/item/matresume-material-cv-resume-vcard-portfolio-html-template/19309564">
            Themeforest
          </a>
        </li>
        <li><h6>Otros</h6>
          <ol style="padding-left: 20px;padding-right: 15px;">
            <li><h6 style="display: inline;">Logo MGUP</h6><a target="_blank" href="https://mgup.cl/logo-320x320-1.png" style="color: #3c3cff;text-decoration: underline;font-size: 85%;" rel="nofollow">(Ver)</a><br> (Censurado) </li>
            <li><h6 style="display: inline;">Video MGUP</h6><a target="_blank" href="logo_mgup.mp4" style="color: #3c3cff;text-decoration: underline;font-size: 85%;" rel="nofollow">(Ver)</a><br> (Censurado) </li>
            <li><h6 style="display: inline;">Imagenes Acuarela</h6><a target="_blank" href="post.html?id=arte_dani" style="color: #3c3cff;text-decoration: underline;font-size: 85%;" rel="nofollow">(Ver)</a><br> (Censurado) </li>
            <li><h6 style="display: inline;">Imagene Galleta</h6><a target="_blank" href="cookie.png" style="color: #3c3cff;text-decoration: underline;font-size: 85%;" rel="nofollow">(Ver)</a><br> (Censurado) </li>
          </ol>
        </li>
      </ol>
    </div>
  </div>
</div>
<div class="row">
    <div class="col-lg-12">
        <div class="mdl-card mdl-shadow--2dp mb-30">
        En el pasado el sitio estaba construido sobre Wordpress PHP, pero a finales del 2021 decidí cambiar el sitio a lo que es actualmente. Aún me falta mucho por ordenar, contenido y nuevas características, pero poco a poco...
        </div>
    </div>
</div>
`;
var juegos_me_gustan=`
<div class="row">
    <div class="col-sm-6 mb-30">
      <div class="mdl-card mdl-shadow--2dp mb-30">
        <h2>Lista</h2>
        <ul style="list-style: inside;">
        <li>Splitgate</li>
        <li>PlayerUnknown&#39;s Battlegrounds (PUBG PC)</li>
        <li>Ajedrez</li>
        <li>RealmRoyale</li>
        <li>OverWatch</li>
        <li>Paladins</li>
        <li>Minimotorways</li>
        <li>Rust</li>
        <li>Left 4 Dead 2</li>
        <li>Uno</li>
        <li>Catan Universe</li>
        <li>Super Smash Bros</li>
        <li>Multiversus</li>
        <li>StarCraft</li>
        <li>Diablo 3</li>
        <li>Minecraft</li>
        <li>Portals</li>
        <li>Team Fortress 2</li>
        <li>Worms Armageddon</li>
        <li>Unreal Tournament 3</li>
        <li>Unreal Tournament 2004</li>
        <li>Lineage 2</li>
        <li>Age Of Mythology: The Titans Expansion</li>
        <li>Counter-Strike 1.6</li> <li>Unreal Tournament</li>
        <li>Tibia</li>
        <li>Diablo 2 Expansion</li>
        <li>Super Mario World</li>
        <li>Cartas Mitos y Leyenda</li>
        </ul>
      </div>
    </div>
    <div class="col-sm-6 mb-30">
      <div class="mdl-card mdl-shadow--2dp mb-30">
        <p>Rara vez grabo y mi habilidad no destaca de forma positiva, pero de todas formas les comparto estos videos seleccionados.</p>
      </div>
      <div class="mdl-card mdl-shadow--2dp mb-30">
        <h2>Playlist de Gameplay</h2>
        <iframe width="100%" height="315" src="https://www.youtube.com/embed/videoseries?list=PLnZnMBzLxULACdKl3Y_Yey1JHbgYUis_F"
        title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
      </div>
    </div>
</div>
<div class="row">
    <div class="col-lg-12">
        <div class="mdl-card mdl-shadow--2dp mb-30">
            <h2>Historia</h2>
            <p>
                En la epoca del colegio, cuando se jugaba cosas como escondidas, murallita, quemadas y ese tipo de juegos,
                mi hermano en casa jugaba Diablo 2, por lo que yo comence a jugarlo tambien (les copiaba todo).
                Luego en el colegio comence a jugar con amigos cosas como Unreal Tournament, Tibia, Cartas Mitos y Leyendas,
                Counter Strike y paginas de mini-juegos uno o dos jugadores como Bubble Trouble, Slither.io, Agar.io, paper.io.
            </p>
            <p>Continuara....</p>
        </div>
    </div>
</div>
`;




var publicaciones_to_show=[
  {
    'post_id':"esta_web",
    'post_img':"cookie.png",
    'post_url':'post.html?id=esta_web',
    'post_target':'',
    'post_date':'2014-06-06T14:38:01Z',
    'post_edit':'2022-08-16T22:44:52Z',
    'post_title':'Sobre Esta Web',
    'post_galeria':'',
    'post_in':'Esta web',
    'zmdi':'favorite',
    'zmdi_color':'bg-hotpink',
    'post_excerpt':'Es mi espacio personal, para mi gusto. Le cambio detalles constantemente poco a poco.',
    'post_text':sobre_esta_web_text
  },
  {
    'post_id':"lucky",
    'post_img':"img/galeria/lucky/a_1_by_unknow.jpg",
    'post_url':'post.html?id=lucky',
    'post_target':'',
    'post_date':'2013-02-10T19:57:30Z',
    'post_edit':'2019-03-19T23:24:33Z',
    'post_title':'Lucky 2013❤2019',
    'post_galeria':'lucky',
    'post_in':'Esta web',
    'zmdi':'favorite',
    'zmdi_color':'bg-hotpink',
    'post_excerpt':'Lo extraño mucho, era muy buen gato. Entendía algunas cosas y muy regalón si lo pillabas de buen humor.',
    'post_text':'<div class="row"><div class="mdl-card mdl-shadow--2dp"><div class="info-wrap">Una personalidad muy calmada, sin contar cuando quería salir y no podía o las clásicas locuras nocturnas de los gatos.</div></div></div>'
  },
  {
    'post_id':"katara",
    'post_img':"img/galeria/katara/IMG_0011.jpg",
    'post_url':'post.html?id=katara',
    'post_target':'',
    'post_date':'2020-06-15T20:27:15Z',
    'post_edit':'2022-12-08T14:12:14Z',
    'post_title':'Katara 2020❤',
    'post_galeria':'katara',
    'post_in':'Esta web',
    'zmdi':'favorite',
    'zmdi_color':'bg-hotpink',
    'post_excerpt':'Katara Luna nació en marzo del 2020 y la adopte en mayo. El primer año vivió acompañándome en la casa de mi familia.',
    'post_text':'<div class="row"><div class="mdl-card mdl-shadow--2dp"><div class="info-wrap">Hoy ya tiene una "hermana" adoptiva con 1 año de diferencia, la <a href="post.html?id=nami" style="color:blue;text-decoration: underline;">Nami</a>.</div></div></div>'
  },
  {
    'post_id':"nami",
    'post_img':"img/galeria/nami/IMG-20210926-WA0000.jpg",
    'post_url':'post.html?id=nami',
    'post_target':'',
    'post_date':'2021-09-27T16:57:42Z',
    //'post_edit':'2022-06-05T14:12:14Z',
    'post_title':'Nami 2021❤',
    'post_galeria':'nami',
    'post_in':'Esta web',
    'zmdi':'favorite',
    'zmdi_color':'bg-hotpink',
    'post_excerpt':'Nami Ñami Chuletas...'
  },
  {
    'post_id':"katara_nami",
    'post_img':"img/galeria/katara_nami/katara-y-nami.jpg",
    'post_url':'post.html?id=katara_nami',
    'post_target':'',
    'post_date':'2022-12-20T18:46:11Z',
    //'post_edit':'2022-06-05T14:12:14Z',
    'post_title':'Galeria Katara y Nami ❤',
    'post_galeria':'katara_nami',
    'post_in':'Esta web',
    'zmdi':'favorite',
    'zmdi_color':'bg-hotpink',
    'post_excerpt':'Aventuras de Katara Luna y Nami Ñami Chuletas...'
  },
  {
    'post_id':"arte_dani",
    'post_img':"img/galeria/arte_dani/a3514c9d-2f31-49e5-9332-d24c32ca7dc5.jpg",
    'post_url':'post.html?id=arte_dani',
    'post_target':'',
    'post_date':'2019-05-23T17:04:49Z',
    //'post_edit':'2022-06-05T14:12:14Z',
    'post_title':'El genial arte de Dani!',
    'post_galeria':'arte_dani',
    'post_in':'Esta web',
    'zmdi':'accounts',
    'zmdi_color':'bg-hotpink',
    'post_excerpt':'A continuación podrán disfrutar de el maravilloso talento de Dani, su habilidad artística, creativa y varias más nunca me dejan de sorprender.',
    'post_text':' <div class="row"><div class="mdl-card mdl-shadow--2dp"><div class="info-wrap">Estoy seguro que conquistara sus metas, es alguien que merece cosas buenas en exceso.</div></div></div>'
  },
  {
      'post_id':"juegos_me_gustan",
      'post_img':'img/galeria/juegos_me_gustan/ai_generated_gamepost_stablediffusionweb_com.jpeg',
      'post_url':'post.html?id=juegos_me_gustan',
      'post_date':'2014-01-15T19:23:42Z',
      'post_edit':'2022-08-15T15:23:42Z',
      'post_title':'Juegos que me gustan',
      'post_in':'Esta web',
      'zmdi':'account',
      'zmdi_color':'bg-hotpink',
      'post_excerpt':'Intentare mantener este post actualizado, contiene una lista e historia de todos los juegos que alguna vez jugue.',
      'post_text':juegos_me_gustan,
    }
];
